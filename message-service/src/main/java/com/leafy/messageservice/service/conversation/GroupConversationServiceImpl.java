...
import com.leafy.messageservice.dto.request.GroupConversationCreateRequest;
import com.leafy.messageservice.dto.request.LeaveGroupRequest;
import com.leafy.messageservice.dto.request.UpdateGroupSettingsRequest;
import com.leafy.messageservice.dto.response.AdminMemberResponse;
import com.leafy.messageservice.dto.response.ConversationResponse;
import com.leafy.messageservice.dto.response.GroupMemberListItemResponse;
import com.leafy.messageservice.dto.response.SearchMemberResponse;
...
public class GroupConversationServiceImpl implements GroupConversationService {
    ...
    public ConversationResponse promoteToAdmin(String conversationId, String targetUserId) {
        ...
        ConversationMember actor = helper.getMemberOrThrow(conversation, currentUserId);
        if (helper.resolveRole(actor) != MemberRole.OWNER) throw new AppException(ErrorCode.SYS_UNCATEGORIZED);

        ConversationMember target = helper.getMemberOrThrow(conversation, targetUserId);
        MemberRole targetRole = helper.resolveRole(target);
        if (targetRole == MemberRole.OWNER) throw new AppException(ErrorCode.SYS_UNCATEGORIZED);
        if (targetRole == MemberRole.ADMIN) throw new AppException(ErrorCode.SYS_UNCATEGORIZED);
    ...
    public ConversationResponse demoteFromAdmin(String conversationId, String targetUserId) {
        ...
        ConversationMember target = helper.getMemberOrThrow(conversation, targetUserId);
        if (helper.resolveRole(target) != MemberRole.ADMIN) throw new AppException(ErrorCode.SYS_UNCATEGORIZED);
    ...
    public PageResponse<List<SearchMemberResponse>> searchMembersToAdd(String conversationId, String query, int page, int size) {
        ...
        if (!nameQuery.isBlank()) {
            candidates.addAll(chatUserRepository.findByFullNameContainingIgnoreCaseAndIdNot(nameQuery, currentUserId, pageable).getContent());
        }
    ...
    public PageResponse<List<GroupMemberListItemResponse>> getGroupMembers(String conversationId, String query, int page, int size) {
        ...
        List<GroupMemberListItemResponse> pageData = dataDocs.stream().map(doc -> {
            ...
            return GroupMemberListItemResponse.builder()
                    ...
                    .isCurrentUser(isCurrentUser).isFriend(false)
    ...
    /* publishGroupMemberEvent method removed */
