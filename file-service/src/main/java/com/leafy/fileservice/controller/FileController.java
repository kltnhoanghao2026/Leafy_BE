...
public class FileController {
                ...
                return s3Service.uploadFile(filePart)
                                .flatMap(s3Response -> {
                                        ...
                                        return fileService.createFile(request)
                                                        .map(response -> {
                                                                log.info("File uploaded and metadata created: fileId={}, s3Key={}, size={}",
                                                                                response.getId(), s3Response.getS3Key(),
                                                                                s3Response.getFileSize());
                                ...
                                .map(fileResponse -> {
                                        ...
                                        return ResponseEntity.ok()
                                                        ...
                                                        .header("Content-Disposition",
                                                                        "attachment; filename=\"" + fileResponse
                                                                                        .getOriginalFileName() + "\"")
                                                        .body(fileData);
                                })
                                .onErrorResume(error -> {
        ...
        public Mono<ResponseEntity<ApiResponse<String>>> generatePresignedUrl(
                ...
                return fileService.getFileById(fileId)
                                .flatMap(fileResponse -> s3Service.generatePresignedUrl(fileResponse.getS3Key(),
                                                expirationMinutes))
                                .map(url -> {
                                ...
                                .onErrorResume(error -> {
                                        ...
                                        return Mono.just(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                                                        .body(ApiResponse.error(9999,
                                                                        "Failed to generate presigned URL", null)));
                                });
        }
         ...
         */
        @PostMapping
        public Mono<ResponseEntity<ApiResponse<FileResponse>>> createFile(
